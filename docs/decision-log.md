...
## D-012 — Arcade leaderboard (daily/weekly/all-time) — DEFERRED
**Status:** ⏸️ Deferred 2026-06-16 — tabled as a future option, not built. Number reserved.
**Sketch (decisions pre-made):** daily/weekly/all-time leaderboard; **arcade 3-initials + anonymous device id, no logins**; **Cloudflare Worker + D1** (a managed edge service deployed separately, so the static GitHub-Pages app stays static); daily-based with weekly/all-time = **best single result**; **store-and-spot-check** integrity (the pure seeded sim lets any score be recomputed offline); **env-gated** (`NEXT_PUBLIC_LEADERBOARD_ENDPOINT`) so the app ships unchanged until configured. This is the P1 feature that D-005/Constitution IV reserved Edge storage for; picking it up = landing that backend. Full design lives in the session plan file.
