...
**Integrity — trust the score (revisited):** the Worker does **not** verify; no roster is sent or stored. Guards: strict field validation, a max body size, one best row per `(device, day)` (unique index + upsert, so a device can't flood), CORS locked to the site origin; per-IP limiting via a Cloudflare dashboard rule. *If abuse appears, on-submit re-simulation is a clean drop-in (the sim is pure + seeded — re-add the roster to the payload and replay `dailySeed(dateKey)`).*
