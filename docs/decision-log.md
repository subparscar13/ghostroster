...
**Amendment 2026-06-19 (two boards + home link):** the leaderboard now has **two boards** — **Daily** (the fair, shared challenge: Today / This week / All-time) and **Regular** (the classic game: All-time / This week, "best season" — not a head-to-head since each classic run is a random franchise+era). A `mode` column (`daily`|`classic`) was added to D1 (unique key now `(device_id, date_key, mode)`); classic submissions carry the run's **own seed**, which the Worker replays to verify high classic claims (daily still derives its seed from the date). A leaderboard link was added to the home screen, and the classic result screen now has the same 3-initials submit box. Frontend stays env-gated/inert.
**Amendment 2026-06-19 (active post prompt):** the submit box (`ScoreSubmit`) was a passive element at the very bottom of the result screen, so few players posted. It's now an **active prompt**: a prominent gold-bordered card moved **directly under the record/grade** (the emotional peak), shown after **every** finished season (daily + classic). When a name is remembered it offers a **one-tap "Post as NAME"** (no typing; first-timers still get the input, auto-focused). Three guardrails: it **never auto-posts** (posting publishes a public name → always one explicit tap), and once a run is posted it becomes a **"Posted ✓ · view board"** confirmation that **persists across reloads/replays** (a small bounded `localStorage` set keyed per run — `daily:{date}` / `classic:{seed}` via `submittedKey`), so it never nags. Still strictly opt-in (FR-014 holds), no login, fully env-gated/inert when unconfigured. Frontend-only — no Worker/sim/data change.
