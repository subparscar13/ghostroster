...
- **FR-023**: System MAY offer a **daily / weekly / all-time leaderboard** for daily-challenge results, with **arcade 3-initials + anonymous device-id identity (no login)** and opt-in submission (FR-014 stays: play needs no account). Backed by a separately-deployed Cloudflare Worker + D1; **env-gated** (`NEXT_PUBLIC_LEADERBOARD_ENDPOINT`) so it ships inert and the static app is unchanged until configured. Scores are trusted (a friends board); weekly/all-time rank each device's best single daily result (D-012).
