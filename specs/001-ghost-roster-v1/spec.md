...
- **FR-018**: The result screen SHOULD show a **season-stats summary** aggregated from the game logs (run differential, runs for/against, team avg, home runs, shutouts, no-hitters, longest streak, biggest win, top performer); the headline feat and the satirical quote MUST **vary across these stats**, not fix on the win streak.
- **FR-017**: System MAY offer an in-app **comment box** (button + modal) anyone can submit without an account, posting to a third-party form service (no backend; constitution IV). It MUST be inert until configured (`NEXT_PUBLIC_COMMENT_ENDPOINT`), private (submissions to the maker, not shown publicly), and note that text goes to a third party (D-010).
- **FR-023**: System MAY offer a **daily / weekly / all-time leaderboard** for daily-challenge results, with **arcade 3-initials + anonymous device-id identity (no login)** and opt-in submission (FR-014 stays: play needs no account). Backed by a separately-deployed Cloudflare Worker + D1; **env-gated** (`NEXT_PUBLIC_LEADERBOARD_ENDPOINT`) so it ships inert and the static app is unchanged until configured. Scores are trusted (a friends board); weekly/all-time rank each device's best single daily result (D-012). Submission is **actively prompted** after every finished season via a prominent inline prompt under the record (one tap when a name is remembered); it **never auto-posts** and, once a run is posted, shows a persistent "Posted ✓" confirmation instead of re-prompting (D-012 amendment 2026-06-19). Classic submissions also carry a `reloads` count — refreshing mid-draft re-spins the current slot (a free re-roll), so a flagged score shows an **asterisk on the win total** on the board (classic only; daily spins are deterministic and unaffected) (D-012 amendment 2026-06-19).
- **FR-022**: The **daily challenge** SHOULD be themed by weekday — each day restricts the spin to one current MLB division's franchises (Sunday = an All-Star pool of each franchise's strongest decade), with re-rolls constrained to that pool. Deterministic from the date (same for everyone); classic mode is unaffected (D-015).
- **FR-021**: The starting pitcher MUST work innings 1–8, then the reliever finishes — except the starter completes the game (and extra innings) while he is still throwing a no-hitter (D-013). No fatigue model (flagged lever). The sim records the per-game starter/reliever innings+runs split so per-pitcher season lines are exact.
- **FR-020**: The box-score screen SHOULD offer a **"Season stats" view** (toggled with the season log) showing per-player accumulated lines: hitters with a full slash line (AB/H/2B/3B/HR/RBI/R/BB/AVG/OBP/SLG/OPS, exact from the batting logs) and pitchers with GS/IP/R/ERA reconstructed from the rotation cycle (starter innings 1–6, reliever 7+). K and W–L are omitted (the engine tracks neither). Derived purely from the existing `SeasonResult`; no sim/data change.
- **FR-019**: System SHOULD offer a player-facing **"how it works" page** (reachable from the footer on every screen) explaining the sim in plain language — the loaded-dice model, era grading, the matchup, and how rolls become runs — for transparency (no hidden ratings). Themed, static, no new dependencies; the technical companion is `docs/how-the-sim-works.md`.
