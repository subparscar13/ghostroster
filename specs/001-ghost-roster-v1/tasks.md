...
- [x] T086 [US2] Flag mid-game-refresh re-spins (D-012 amend) — classic refresh/resume mid-draft re-spins the current slot (free re-roll, bypassing the 2+2 caps); `RunContainer` counts mid-draft classic resumes into `PersistedRun.reloads` (persists on the first spin too, to close the pre-first-draft gap); threaded → `ResultScreen`/`ScoreSubmit` → `buildSubmission` (`Submission.reloads`, classic only — daily is deterministic, never flagged). Worker: `reloads` column (additive migration) + INSERT/upsert + board select; `LeaderboardView` renders an asterisk on the win total (`158*–4`) + a legend. Client-reported honesty marker (wins still server-verified). +buildSubmission tests; SQL cross-checked.
